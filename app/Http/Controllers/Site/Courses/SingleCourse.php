...
class SingleCourse extends BaseComponent
{
    ...
    public  $course;
    public  $related_courses = [] , $comments = [] ,
    // Mahdi
    //  $recaptcha ,
      $episodes , $user , $commentCount = 10 , $actionComment  , $actionLabel = 'دیدگاه جدید';
...
//        }
    ...
    // ✅ فقط کامنت‌های عادی (general)
    $this->comments = Comment::where('commentable_type', 'App\Models\Course')
        ->where('commentable_id', $this->course->id)
        ->where('type', Comment::TYPE_GENERAL)
   ...
   public function set_content($type,$id)
    ...
    // ✅ استفاده مستقیم از Comment::create با تمام فیلدهای اجباری
    Comment::create([
        'user_id' => auth()->id(),
        ...
        'status' => $status,
        'type' => Comment::TYPE_GENERAL,
    ]);

    $this->reset(['comment', 'actionLabel', 'actionComment']);

    // ✅ بارگذاری مجدد با متد صحیح (نه از رابطه Eloquent)
    $this->loadCourse();
    ...
    // // استفاده مستقیم از Comment::create برای اطمینان از پر شدن تمام فیلدهای اجباری
    // Comment::create([
    //     'user_id' => auth()->id(),
    //     'commentable_type' => 'App\Models\Course',
    //     'commentable_id' => $this->course->id,
...
// endmahdi
    ...
    public $studentCommentCount = 5;
    public $actionStudentComment = null;


    public function loadStudentComments()
    {
        // if (!isset($this->course)) {
        //     return;
        // }

        // $this->student_comments = Comment::where('commentable_type', Course::class)
        //     ->where('commentable_id', $this->course->id)
        //     ->where('type', 'student') // بعد از اضافه کردن فیلد type
        //     ->where('status', CommentEnum::CONFIRMED)
        //     ->with(['user', 'childrenRecursive' => function($q) {
        //         return $q->where('status', CommentEnum::CONFIRMED);
        //     }])
    ...
    $this->student_comments = Comment::where('commentable_type', 'App\Models\Course')
        ->where('commentable_id', $this->course->id)
        ->where('type', Comment::TYPE_STUDENT)  // فقط نظرات هنرجویان
        ->where('status', CommentEnum::CONFIRMED)
        ->with(['user', 'childrenRecursive' => function($q) {
            return $q->where('status', CommentEnum::CONFIRMED);
        }])
    ...
    {
        // $this->validate([
        //     'student_comment' => 'required|min:3|max:1000',
        // ]);

        // // بررسی اینکه کاربر دوره رو خریده باشه
        // if (!auth()->user()->hasCourse($this->course->id)) {
        //     $this->addError('student_comment', 'شما دسترسی به ثبت نظر هنرجویی ندارید');
        //     return;
        // }
        ...
        //     'commentable_id' => $this->course->id,
        //     'content' => $this->student_comment,
        //     'status' => CommentEnum::CONFIRMED,
        //     'type' => 'student',
        //     'parent_id' => $this->actionStudentComment ?? null,
        // ]);

        // // ریست کردن فیلدها
        // $this->student_comment = '';
        // $this->actionStudentComment = null;

        // // بارگذاری مجدد نظرات
        // $this->loadStudentComments();

        // $this->emitNotify('نظر شما با موفقیت ثبت شد', 'success');



    //     $this->validate([
    //     'student_comment' => 'required|string|min:3|max:1000',
    // ], [], [
    //     'student_comment' => 'متن نظر'
    // ]);

    // if (!auth()->check()) {
    //     return $this->addError('student_comment', 'لطفا ابتدا وارد شوید');
    // }

    // if (!$this->user->hasCourse($this->course->id)) {
    //     $this->addError('student_comment', 'شما دسترسی به ثبت نظر هنرجویی ندارید. ابتدا این دوره را خریداری کنید.');
    //     return;
    // }

    // // همیشه وضعیت رو CONFIRMED بذار (برای تست در لوکال)
    // $status = CommentEnum::CONFIRMED;
    ...
    // return $this->emitNotify('نظر هنرجویی شما با موفقیت ثبت شد');
     $this->validate([
    ...
    if (!auth()->check())
        return $this->addError('student_comment', 'لطفا ابتدا ثبت نام کنید');

    if (!$this->user->hasCourse($this->course->id))
        return $this->addError('student_comment', 'شما دسترسی به ثبت نظر هنرجویی ندارید.');

    $status = CommentEnum::CONFIRMED;

    // ✅ استفاده مستقیم از Comment::create
    Comment::create([
        'user_id' => auth()->id(),
        'commentable_type' => 'App\Models\Course',
        'commentable_id' => $this->course->id,
    ...
    $this->reset(['student_comment', 'actionStudentComment']);

    // ✅ بارگذاری مجدد نظرات هنرجویان
    $this->loadStudentComments();
    // ✅ بارگذاری مجدد نظرات عادی
    $this->loadCourse();

    return $this->emitNotify('نظر هنرجویی شما با موفقیت ثبت شد');
    }

    public function moreStudentComment()
    {
        $this->studentCommentCount += 5;
    }



public function updatedActionStudentComment($value)
{
    // این رو پر کن:
    $this->actionLabel = $value ? 'ارسال پاسخ' : 'دیدگاه جدید';
}
